...
import { resetPartnerPassword } from "@/src/actions/authActions";
...
export default function PartnerResetScreen() {
  ...
  const handleReset = async () => {
    ...
    setError(null);
    try {
      await resetPartnerPassword({ email: email.trim() });
      Alert.alert("메일 전송 완료", "비밀번호 재설정 메일을 확인해 주세요.");
      router.replace("/(partner)/auth/login");
    } catch (err) {
      console.error("[partner][auth] reset error", err);
  ...
  return (
    <Screen scroll style={styles.container}>
      ...
      <Card style={styles.card}>
        ...
        <SecondaryButton
          label="로그인으로"
          onPress={() => router.replace("/(partner)/auth/login")}
        />
      </Card>
    </Screen>
  );
