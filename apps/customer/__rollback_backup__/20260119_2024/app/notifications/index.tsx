...
export default function CustomerNotificationsScreen() {
  ...
  useEffect(() => {
    const unsub = subscribeNotifications({
      ...
      onError: (err) => {
        console.error("[customer][notifications] subscribe error", err);
  ...
  const handleOpen = async (item: NotificationDoc) => {
    if (uid && !item.read) {
      markNotificationRead(uid, item.id).catch((err) => {
        console.error("[customer][notifications] mark read error", err);
      });
    }

    const data = item.data as Record<string, unknown> | undefined;
    if (item.type === "quote_received" && data?.requestId) {
  ...
  const content = useMemo(() => {
    if (!items.length) {
      return (
        <EmptyState
          title="새 알림이 없습니다."
          description="견적이나 채팅 알림이 도착하면 알려드릴게요."
        />
      );
    }
