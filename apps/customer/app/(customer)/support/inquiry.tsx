...
export default function CustomerInquiryScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert("문의 접수 완료", "빠르게 확인 후 연락드릴게요.");
    } catch (err) {
      console.error("[customer][support] create error", err);
    ...
    <Screen style={styles.container} contentContainerStyle={styles.content}>
      ...
      <Card style={styles.card}>
        ...
        <PrimaryButton
          ...
          disabled={saving || status === "authLoading"}
        />
        <SecondaryButton label="운영시간 보기" onPress={() => {}} disabled />
      </Card>
    </Screen>
  );
}
