...
export async function upsertChatNotification(input: {
  ...
  try {
    ...
    if (existing) {
      ...
      const createdAt = data.createdAt as unknown as { toMillis?: () => number };
      const now = Date.now();
      const last = createdAt?.toMillis ? createdAt.toMillis() : 0;
      if (now - last < 30_000) {
    ...
    console.warn("[partner][notifications] chat dedupe error", error);
  }
