...
exports.createPartnerAdBid = void 0;
...
function getNextWeekRange(base) {
...
exports.createPartnerAdBid = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
    ...
    if (!Number.isFinite(amount) || amount < MIN_BID_POINTS) {
        throw new functions.https.HttpsError("invalid-argument", `최소 ${MIN_BID_POINTS.toLocaleString("ko-KR")}포인트 이상 필요합니다.`);
    }
    const now = getSeoulDate();
    if (isBidClosed(now)) {
        throw new functions.https.HttpsError("failed-precondition", "일요일 22시 이후에는 입찰할 수 없습니다.");
    }
