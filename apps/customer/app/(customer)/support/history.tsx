...
export default function SupportHistoryScreen() {
  ...
  const loadTickets = async (isRefresh = false) => {
    ...
    try {
      ...
      console.error("[customer][support] load tickets error", err);
  ...
  return (
    <Screen scroll={false} style={styles.container}>
      <View style={styles.headerWrap}>
        <AppHeader title="문의 내역" subtitle="내가 남긴 문의와 답변을 확인하세요." />
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            ...
            onPress={() => router.push(`/(customer)/support/${item.id}`)}
          >
            <Card style={styles.card}>
              ...
              <Text style={styles.cardMeta}>
                접수 {item.createdAt ? formatTimestamp(item.createdAt) : "-"}
              </Text>
              <Text style={styles.cardMeta}>
                업데이트 {item.updatedAt ? formatTimestamp(item.updatedAt) : "-"}
              </Text>
            </Card>
          </TouchableOpacity>
        )}
