...
function setCors(res: functions.Response, origin = "*") {
  res.set("Access-Control-Allow-Origin", origin);
  res.set("Access-Control-Allow-Methods", "POST,OPTIONS");
  res.set("Access-Control-Allow-Headers", "Content-Type, Authorization");
}

async function readJsonBody(req: functions.Request) {
...
async function issueFirebaseToken(profile: ProviderProfile) {
  if (!profile.id) {
...
export const authKakao = functions.https.onRequest(async (req, res) => {
  setCors(res);
...
export const authNaver = functions.https.onRequest(async (req, res) => {
  setCors(res);
  if (req.method === "OPTIONS") {
    res.status(204).send("");
    return;
